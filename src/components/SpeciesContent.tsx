...
const lifeHedge = [
  ...
  "Available fossils suggest",
  "Most paleontologists agree",
  "Studies of related species imply",
];
const sciHedge = [
  "Current data indicate",
  "Analyses suggest",
  "Available evidence implies",
  "Comparative studies show",
  "Recent re-examinations confirm",
];
...
function dietPhrase(diet: string): string {
  switch (diet) {
    case "Carnivore":
      return "an active predator that pursued or ambushed live prey";
    case "Herbivore":
      return "a plant-eater that browsed or grazed across its environment";
    case "Omnivore":
      return "an opportunistic feeder consuming both plant matter and small animals";
    case "Piscivore":
      return "a fish-eater specialised for capturing aquatic prey";
    case "Insectivore":
      return "a small-bodied feeder that hunted insects and invertebrates";
    default:
      return "a feeder adapted to its local food resources";
  }
}
...
function periodContext(p: string): string {
  switch (p) {
    case "Permian":
      return "the late Paleozoic, before true dinosaurs existed, when synapsids and early reptiles dominated terrestrial ecosystems";
    case "Triassic":
      return "an experimental phase of archosaur evolution, recovering from the largest mass extinction in Earth\u2019s history";
    case "Jurassic":
      return "the first great age of dinosaur dominance, with sprawling forests of conifers, cycads and tree ferns";
    case "Cretaceous":
      return "the longest dinosaur-dominated period, marked by the rise of flowering plants and increasingly complex food webs";
    default:
      return "a deep-time interval of reptile diversification";
  }
}
...
function climateNote(p: string): string {
  switch (p) {
    case "Permian":
      return "Climates were strongly seasonal, with vast inland deserts and a single supercontinent (Pangaea) limiting moisture inland.";
    case "Triassic":
      return "Global climates were warm and dry overall, with monsoonal margins and few polar ice deposits.";
    case "Jurassic":
      return "Conditions were warm and humid, with no permanent polar ice and broad shallow seas across the continents.";
    case "Cretaceous":
      return "The world was warmer than today, with high sea levels flooding continental interiors and creating extensive coastal habitats.";
    default:
      return "Local climate exerted strong control over what species could persist.";
  }
}

function intelligencePhrase(score: number): string {
  if (score >= 8)
    return "high \u2014 among the more cognitively capable archosaurs of its time";
...
function speedPhrase(score: number, taxon: string): string {
  if (taxon === "pterosaur")
    return score >= 6
      ? "a strong, sustained flier capable of long-distance travel"
      : "a competent but short-burst flier";
  if (taxon === "marine_reptile")
    return score >= 6
      ? "a fast cruising swimmer capable of pursuit"
      : "a slower, ambush-style swimmer";
  return score >= 7
    ? "a fast runner relative to its body mass"
    : score >= 4
      ? "a moderate runner with bursts of speed"
      : "a slow, steady mover";
}

function bitePhrase(score: number, diet: string): string {
  if (diet === "Herbivore")
    return "cropping or grinding teeth suited to plant material";
  if (score >= 8)
    return "one of the most powerful bites in its ecosystem, capable of crushing bone";
...
function defensePhrase(score: number, features: string[]): string {
  const armor = features.find((f) =>
    /armor|spike|plate|shield|frill|club|horn/i.test(f),
  );
  if (armor) return `passive defence centred on its ${armor.toLowerCase()}`;
  if (score >= 7)
    return "robust skeletal architecture and bulk providing strong passive defence";
  return "limited passive defence \u2014 likely relied on agility or group behaviour";
}

function locomotionContext(taxon: string): string {
  if (taxon === "pterosaur")
    return "A quadrupedal launch posture is now favoured by most experts, with powerful forelimb muscles providing the initial vault into flight";
  if (taxon === "marine_reptile")
    return 'Underwater locomotion was driven by either flipper-based "underwater flying" or lateral undulation of the body and tail, depending on lineage';
...
function buildLifeSections(d: Dinosaur): Section[] {
  ...
  const behaviorBullets = d.distinctFeatures
    .slice(0, 3)
    .map((f) => `\u2022 ${f}`)
    .join("\n");
  const behavior = [
  ...
  return [
    {
      ...
      placeholderLabel: `${d.name} Reconstruction`,
    },
    {
      id: "habitat",
...
function buildScientificSections(d: Dinosaur): Section[] {
  ...
  return [
    ...
    {
      id: "muscle",
      title: "Muscle Reconstruction & Locomotion",
      body: muscle,
      placeholder: "anatomy",
    },
    {
    ...
    {
      id: "bite",
      title: "Feeding Mechanics & Bite Force",
      body: bite,
      placeholder: "bite",
    },
    {
      id: "strength",
      title: "Physical Strength & Durability",
      body: strength,
      placeholder: "strength",
    },
    {
      id: "intel",
      title: "Intelligence & Senses",
      body: intel,
      placeholder: "intelligence",
    },
    {
      id: "fossil",
      title: "Fossil Evidence & Pathologies",
      body: fossil,
      placeholder: "fossil",
    },
  ];
...
function buildLifeFunFacts(d: Dinosaur): string[] {
  ...
  if (ageMid <= 70) {
    facts.push(
      `Lived around ${ageMid} million years ago \u2014 closer in time to humans than to Stegosaurus, which preceded it by over 80 million years.`,
    );
  } else if (ageMid >= 200) {
    facts.push(
      `Roamed the planet roughly ${ageMid} million years ago, before flowering plants existed and when forests were dominated by conifers, ferns and cycads.`,
    );
  } else {
    facts.push(
      `Lived approximately ${ageMid} million years ago, deep in the ${d.period} period and long before any modern mammal lineage appeared.`,
    );
  }

  if (taxon === "pterosaur") {
    facts.push(
      `Despite being a contemporary of dinosaurs, ${d.name} was not a dinosaur \u2014 pterosaurs belong to a separate flying-reptile lineage and evolved powered flight independently.`,
    );
  } else if (taxon === "marine_reptile") {
    facts.push(
      `Although it shared the world with dinosaurs, ${d.name} was not a dinosaur \u2014 it belonged to a separate reptile lineage that returned to the sea.`,
    );
  }

  if (d.weight >= 5000) {
    facts.push(
      `Outweighed roughly ${(d.weight / 6000).toFixed(1)} adult African elephants \u2014 a true heavyweight of its ecosystem.`,
    );
  } else if (d.weight <= 30) {
    facts.push(
      `Weighed less than a modern Border Collie, despite living alongside far larger contemporaries.`,
    );
  } else if (d.length >= 25) {
    facts.push(
      `Stretched over ${d.length} metres from snout to tail tip \u2014 longer than a typical city bus.`,
    );
  }

  if (d.distinctFeatures[0]) {
    facts.push(
      `One of its most striking features is ${d.distinctFeatures[0].toLowerCase().replace(/\.$/, "")}, a trait that sets it apart from most contemporaries.`,
    );
  }

  facts.push(
    `Formally described in ${d.discovery.year} from fossils unearthed in ${d.discovery.location.split(",").slice(-1)[0].trim()}.`,
  );
...
function buildScientificFunFacts(d: Dinosaur): string[] {
  ...
  if (skel.completeness >= 80) {
    facts.push(
      `At ${skel.completeness}% skeletal completeness, ${d.name} is among the better-known species in the fossil record \u2014 most extinct reptiles are reconstructed from far less.`,
    );
  } else if (skel.completeness <= 30) {
    facts.push(
      `Only about ${skel.completeness}% of its skeleton has ever been recovered, so much of the reconstruction relies on phylogenetic bracketing against close relatives.`,
    );
  } else {
    facts.push(
      `Roughly ${skel.completeness}% of the skeleton is known from fossil material, a typical figure for medium-coverage extinct species.`,
    );
  }

  facts.push(
    `First scientifically named in ${d.discovery.year}, the holotype specimen has been re-examined repeatedly as new analytical techniques have become available.`,
  );
  facts.push(
    `Belongs to the family ${d.classification.family}, within the broader ${d.group} group \u2014 a placement supported by both skeletal traits and modern phylogenetic analysis.`,
  );

  if (d.combatStats.biteForce >= 8 && d.diet !== "Herbivore") {
    facts.push(
      `Bite-force estimates place ${d.name} among the most powerful jawed predators of its size class, with crushing strength capable of fracturing prey bone.`,
    );
  } else if (d.combatStats.intelligence >= 7) {
    facts.push(
      `Endocast studies suggest a relatively enlarged brain for its body size, hinting at more complex behavioural repertoires than many contemporaries.`,
    );
  } else if (taxon === "pterosaur" && d.wingspan && d.wingspan >= 6) {
    facts.push(
      `With an estimated wingspan of around ${d.wingspan} m, biomechanical models suggest it could remain airborne for hours using thermal soaring.`,
    );
  } else if (taxon === "marine_reptile") {
    facts.push(
      `Bone microstructure suggests adaptations for sustained diving, including dense, ballast-like ribs and modifications of the limb bones into flippers.`,
    );
  } else {
    facts.push(
      `Limb proportions place ${d.name} among the ${d.combatStats.speed >= 7 ? "more cursorial" : "more graviportal"} members of its group.`,
    );
  }

  facts.push(
    `Stratigraphic range falls within the ${d.period} period (${d.periodRange.end}\u2013${d.periodRange.start} Mya), constraining its appearance in the geological record.`,
  );
  return facts.slice(0, 5);
}
...
function CinematicBrackets({ color = "amber" }: { color?: "amber" | "muted" }) {
  ...
  return (
    <>
      <div
        className={`absolute top-2 left-2 w-5 h-5 border-t border-l ${cls} pointer-events-none`}
      />
      <div
        className={`absolute top-2 right-2 w-5 h-5 border-t border-r ${cls} pointer-events-none`}
      />
      <div
        className={`absolute bottom-2 left-2 w-5 h-5 border-b border-l ${cls} pointer-events-none`}
      />
      <div
        className={`absolute bottom-2 right-2 w-5 h-5 border-b border-r ${cls} pointer-events-none`}
      />
    </>
  );
}
...
function ExhibitHeader({
  ...
  return (
    <div className="space-y-3 relative">
      ...
      <div
        className="absolute -top-2 right-0 text-[80px] md:text-[100px] font-display font-black tabular-nums select-none pointer-events-none leading-none"
      ...
      <h2
        className={`text-2xl md:text-3xl lg:text-[2rem] font-display font-bold tracking-tight pl-4 border-l-2 leading-tight transition-all duration-500 ${
          isActive
    ...
    const map: Record<string, { badge: string; rows: [string, string][] }> = {
      ...
      habitat: {
        ...
        rows: [
          ...
          [
            "Climate",
            dino.period === "Cretaceous"
              ? "Warm, no polar ice"
              : dino.period === "Jurassic"
                ? "Warm & humid"
                : "Arid–seasonal",
          ],
          ["Period", dino.period],
      ...
      behavior: {
        ...
        rows: [
          [
            "Speed Class",
            sp >= 7 ? "Cursorial" : sp >= 4 ? "Moderate" : "Graviportal",
          ],
          ["Speed Index", `${sp} / 10`],
      ...
      social: {
        badge: "BEHAVIORAL DATA",
        rows: [
          ["Intelligence", `${iq} / 10`],
          [
            "Social Index",
            iq >= 7 ? "Complex" : iq >= 5 ? "Moderate" : "Minimal",
          ],
          ["Group Size", iq >= 7 ? "Likely social" : "Likely solitary"],
          [
            "Brain Class",
            iq >= 7 ? "High EQ" : iq >= 5 ? "Medium EQ" : "Low EQ",
          ],
        ],
      },
      repro: {
        badge: "REPRODUCTIVE DATA",
        rows: [
          [
            "Strategy",
            getTaxonomyType(dino) === "marine_reptile"
              ? "Viviparous"
              : "Oviparous",
          ],
          ["Group", dino.group],
  ...
  const sciMap: Record<string, { badge: string; rows: [string, string][] }> = {
    skeletal: {
      ...
      rows: [
        ...
        [
          "Holotype",
          skel.completeness >= 70
            ? "Good"
            : skel.completeness >= 40
              ? "Partial"
              : "Fragmentary",
        ],
    ...
    muscle: {
      ...
      rows: [
        [
          "Muscle Mass Est.",
          `${Math.round(((dino.weight * 0.38) / 1000) * 10) / 10} t`,
        ],
        [
          "Build Type",
          sz >= 7 ? "Graviportal" : sp >= 6 ? "Cursorial" : "Generalist",
        ],
        ["Speed Index", `${sp} / 10`],
        ["Size Index", `${sz} / 10`],
      ],
    ...
    bite: {
      badge: "BITE FORCE DATA",
      rows: [
        [
          "Force Class",
          bf >= 8 ? "Extreme" : bf >= 6 ? "High" : bf >= 4 ? "Moderate" : "Low",
        ],
        ["Force Index", `${bf} / 10`],
        ["Diet", dino.diet],
    ...
    strength: {
      ...
      rows: [
        ...
        [
          "Known Pathologies",
          dino.combatStats.defense >= 5 ? "3–5 types" : "1–3 types",
        ],
        ["Specimen Count", skel.completeness >= 70 ? "Multiple" : "Single"],
      ],
    },
    intel: {
      badge: "NEUROLOGY DATA",
      rows: [
        ["Intelligence", `${iq} / 10`],
        ["EQ Class", iq >= 7 ? "High" : iq >= 5 ? "Medium" : "Low"],
        [
          "Primary Sense",
          dino.diet === "Carnivore" ? "Vision + Olfaction" : "Vision (wide)",
        ],
        ["Diet", dino.diet],
      ],
    },
    fossil: {
      badge: "DISCOVERY DATA",
      rows: [
        ["Described By", dino.discovery.discoverer],
        ["Year", String(dino.discovery.year)],
        [
          "Location",
          dino.discovery.location.split(",").slice(-2).join(",").trim(),
        ],
        ["Completeness", `${skel.completeness}%`],
      ],
    },
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-2">
        <span className="text-[8px] uppercase tracking-[0.2em] font-display text-muted-foreground/40">
          Skeletal Record
        </span>
        <span className="text-[9px] font-mono tabular-nums text-amber-400/60">
          {skel.completeness}% KNOWN
        </span>
      </div>
    ...
    <div className="relative rounded-lg overflow-hidden group">
      ...
      {scanLabel && (
        <div className="absolute bottom-0 left-0 right-0 px-3 py-1.5 bg-gradient-to-t from-black/60 to-transparent">
          <span className="text-[8px] uppercase tracking-[0.18em] font-display text-amber-400/60">
            {scanLabel}
          </span>
        </div>
      )}
      ...
      <div className="space-y-3">
        ...
        <h2 className="text-2xl md:text-3xl font-display font-bold tracking-tight pl-4 border-l-2 border-amber-500/30 text-foreground flex items-center gap-3">
          ...
          {mode === "life"
            ...
            : "Scientific Notes from the Field"}
        </h2>
      </div>
        ...
        {facts.map((f, i) => (
          ...
          >
            ...
            <div className="flex-shrink-0 h-6 w-6 rounded-sm bg-amber-500/10 border border-amber-500/20 flex items-center justify-center mt-0.5">
              <span className="text-[9px] font-display font-bold text-amber-400/70 tabular-nums">
