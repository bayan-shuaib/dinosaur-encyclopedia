...
const W = 500;
const H = 260;
const MARKER_COLOR = 'hsl(15, 75%, 60%)';
...
export function LocationMapSingle({ location, continent }: Props) {
  ...
  const pinY = markerPos.y - 14;

  return (
    <div className="mt-2 rounded-lg overflow-hidden" style={{ background: 'hsl(0,0%,4%)' }}>
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full h-auto block">
        <defs>
          <clipPath id="robinson-clip-single">
            <polygon points={outline} />
          </clipPath>
          <filter id="pin-glow-single" x="-100%" y="-100%" width="300%" height="300%">
        ...
        {/* Grid */}
        <g clipPath="url(#robinson-clip-single)" opacity="0.1">
          {grid.lats.map((pts, i) => (
        ...
        {/* Glow pulse */}
        <circle cx={markerPos.x} cy={markerPos.y} r={12} fill={MARKER_COLOR} opacity={0.1}>
          <animate attributeName="r" values="10;18;10" dur="2.4s" repeatCount="indefinite" />
          <animate attributeName="opacity" values="0.14;0.04;0.14" dur="2.4s" repeatCount="indefinite" />
        </circle>

        {/* Pin shadow */}
        <ellipse cx={markerPos.x} cy={markerPos.y + 2} rx={4} ry={1.8} fill="hsl(0,0%,0%)" opacity={0.25} />

        {/* Pin stem */}
        <line
          x1={markerPos.x} y1={markerPos.y}
          x2={markerPos.x} y2={pinY + 9}
          stroke={MARKER_COLOR}
          strokeWidth={1.6}
          opacity={0.9}
        />
        ...
        {/* Pin head */}
        <circle cx={markerPos.x} cy={pinY} r={5} fill={MARKER_COLOR} filter="url(#pin-glow-single)" />
        <circle cx={markerPos.x} cy={pinY} r={2} fill="hsl(0,0%,100%)" opacity={0.65} />
      </svg>
    </div>
  );
}
